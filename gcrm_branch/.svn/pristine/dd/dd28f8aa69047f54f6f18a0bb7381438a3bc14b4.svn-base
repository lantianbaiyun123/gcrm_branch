  ...
  function ($q, $timeout, $state, AdState, AdPrivate, AdHttp, AdBasic, AdContent, AdContentUtil, AdConstant, PageSet, Select2Suggest, GCRMUtil) {
    return {
      ...
      updateState: function (scope) {
        ...
        scope.state.canRemoveSolution = AdState.canRemoveSolution(scope);
        scope.state.canCreatePo = AdState.canCreatePo(scope);
        scope.state.canCreateSingle = AdState.canCreateSingle(scope);
        scope.state.canBusinessChange = AdState.canBusinessChange(scope);
        scope.state.canCancelSingle = AdState.canCancelSingle(scope);
      },
      /**
       * 添加一个广告内容
       * @param {[type]} scope [description]
    ...
    return {
        ...
        return (
          ...
          (
            ...
            (
              ...
              (scope.basic.adSolution.approvalStatus === 'approved' && ifContract)
            )
          )
        );
      },
      ...
      canSolutionChange: function (scope) {
        return (
          ...
          scope.basic.solutionStatus == 'effective' &&
      ...
      canRemoveSolution: function (scope) {
        var basic = scope.basic;
        return (
          (!scope.OwnerOpers.btn_adsol_detail_adsol_del || basic.isOwner) &&
          scope.BtnIndex.btn_adsol_detail_adsol_del &&
          basic.canDelete
        );
      },
      canCreatePo: function (scope) {
        var basic = scope.basic;
        return (
          (!scope.OwnerOpers.btn_adsol_detail_po_create || basic.isOwner) && // isOwner权限控制
          scope.BtnIndex.btn_adsol_detail_po_create &&  // 用户功能权限控制
          basic.canCreatePO &&  // ??
          basic.solutionStatus == 'approved' && // 方案状态 === '审核通过‘
          !scope.adContents[0].adSolutionContent.poNum && // 无PO，通过方案下第1个内容有无PO判断
          !basic.adSolution.contractType && // 方案无合同类型
          ( ( !basic.adSolution.contractStatus ) || (  basic.adSolution.contractStatus === 'Revoked' && !basic.contract) ) && // 无方案合同状态 或 方案合同状态为“已撤销”且合同对象为空
          (!(basic.contract && basic.contract.data) || (basic.contract.data && basic.contract.data.state === 'VALID') ) &&  // 合同对象为空 或 合同状态为已生效
          !basic.oldSolutionNumber  // 无原方案编号
        );
      },
      canCreateSingle: function (scope) {
        var basic = scope.basic;
        return (
          (!scope.OwnerOpers.btn_adsol_detail_submit_to_busi || basic.isOwner) && // isOwner权限控制
          scope.BtnIndex.btn_adsol_detail_submit_to_busi && // 用户功能权限控制
          basic.solutionStatus == 'approved' && // 方案状态 === '审核通过‘
          !basic.adSolution.contractType && // 无合同类型
          (!basic.adSolution.contractStatus || basic.adSolution.contractStatus=='Revoked') && // 无方案合同状态 或 方案合同状态为“已撤销”
          !(basic.contract && basic.contract.data) && //合同对象为空
          !basic.oldSolutionNumber && // 无原方案编号
          basic.advertiseType !== 'zerotest'  //不是 0圆测试单
        );
      },
      canBusinessChange: function (scope) {
        var basic = scope.basic;
        return (
          (!scope.OwnerOpers.btn_adsol_detail_submit_to_busi || basic.isOwner) &&  // isOwner权限控制
          scope.BtnIndex.btn_adsol_detail_submit_to_busi && // 用户功能权限控制
          ...
          basic.oldSolutionNumber &&  // 无原方案编号
          !basic.adSolution.contractType && // 无合同类型
          (!(basic.contract && basic.contract.data) || !basic.contract.data.number) // 无合同编号
        );
      },
      canCancelSingle: function (scope) {
        var basic = scope.basic;
        return (
          (!scope.OwnerOpers.btn_adsol_detail_submit_to_busi || basic.isOwner) &&  // isOwner权限控制
          scope.BtnIndex.btn_adsol_detail_submit_to_busi && // 用户功能权限控制
          basic.solutionStatus == 'approved' && // 方案状态 === '审核通过‘
          basic.adSolution.contractType &&  // 合同类型非空
          ( !(basic.contract && basic.contract.data) || basic.contract.data.state == 'VALID' ) && // 合同对象为空 或 合同状态为已生效
          basic.adSolution.contractStatus == 'display' && // 方案合同状态为“display”
          !scope.adContents[0].adSolutionContent.poNum  // 无PO，通过方案下第1个内容有无PO判断
        );
      }
    };
