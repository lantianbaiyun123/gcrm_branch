...
public class PublishServiceImpl implements IPublishService {
	...
	public void terminateAll(String number, Long operatorId) {
        ...
        dateRelationService.removeRelationsAndReleaseStocksFromDate(publish.getAdContentId(),
                DateUtils.getCurrentDateOfZero());
        scheduleService.releaseScheduleByAdContentId(publish.getAdContentId());
	...
	public void autoPublishAfterChangePO(AdSolutionContent content) {
		...
		Schedules schedule = scheduleService.findByNumber(oldPublish.getScheduleNumber());
		if (schedule == null) {
	...
	@Override
	public Publish createPublish(AdSolutionContent content, Schedules schedule, String materialNumber) {
		try {
	...
	private List<PublishDate> saveAndGetPublishDates(AdSolutionContent content, Schedules schedule, Publish publish) {
	...
	public boolean tryLockScheduleAndCreatePublish(AdSolutionContent content) {
		...
		List<Schedules> schedules = scheduleService.findByAdContentIdAndStatus(content.getId(), ScheduleStatus.confirmed);
		if (CollectionUtils.isEmpty(schedules)) {
			LoggerHelper.err(getClass(), "生成上线单时发现广告内容没有已确认的排期单，广告内容编号：{}", content.getNumber());
			return false;
		}
		Schedules schedule = schedules.get(0);
