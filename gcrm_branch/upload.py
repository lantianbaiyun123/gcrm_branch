...
# WITHOUT WARRANTIES OR CONDITIONSre OF ANY KIND, either express or implied.
...
module_space ={'app/ecom/adx/trunk/win-notice-server':'winnotice-server',
               'app/ecom/adx/trunk/server':'adx-server',
               'app/ecom/adx/trunk/pre-targeting-lib':'pre-targeting',
               'app/ecom/adx/trunk/cookie-matching-serv':'cookie-matching',
               'app/ecom/adx/trunk/buyer-adaptor':'buy-adaptor',
               'app/ecom/ubmc':'nova',
               'app/ecom/nova/trunk/hc':'ecom-hc',
               'app/ecom/nova/trunk/afs':'nova',
               'app/ecom/nova/trunk/ctr':'nova',
               'app/ecom/nova/trunk/dc':'nova',
               'app/ecom/nova/trunk/fe':'nova',
               'app/ecom/nova/trunk/kr':'nova',
               'app/ecom/nova/trunk/pfs':'nova',
               'app/ecom/nova/trunk/re':'nova',
               'app/ecom/nova/trunk/se/se-as':'nova',
               'app/ecom/nova/trunk/se/se-bs':'nova',
               'app/ecom/nova/trunk/se/se-cs':'nova',
               'app/ecom/ufs':'nova',
               'app/ecom/nova/trunk/ui':'nova',
               'app/ecom/nova/trunk/kr':'nova',
               'app/ecom/nova/trunk/public/streamfold':'nova',
               'app/ecom/nova/trunk/promc':'nova',
               'app/ecom/nova/trunk/nebula':'nova',
               'app/ecom/uap/trunk':'uap',
               'app/ecom/dn/trunk':'dnbackend',
               'iit-test/trunk/data-engine':'cr'}
BACK_GROUND_REVERSE ='\033[7m'
...
def find_space(svn_url): 
    for key in module_space:
        if svn_url.find(key)==-1:
            continue
        return module_space[key]
...
OWNER_FILE_LOCATION = os.getcwd().replace('\\','/') + "/owner.txt"
DEFAULT_LIST_NUM = 10
ANALYSIS_RESULT_FOLDER_PATH = os.getcwd().replace('\\','/')+'/cooder'
...
parser = optparse.OptionParser( 
    ...
    epilog = "For more help, please visit:  http://wiki.babel.baidu.com/twiki/bin/view/Project/Cr/Upload脚本使用说明" ,
...
group.add_option( "--co", action = "store", dest = "check_out", default = None,
                 help = "Check out the new versions of a working copy from a repository. "
                 "For example: 'python upload.py --co=153295'." )
...
group.add_option( "--story", action = "store",
                 dest = "card", default = None,
                 help = ( "Feature is icafe+'s card, just input the space name + card id, like #cr-56."   ) )
group.add_option( "--search_card", action = "store_true",
                 dest = "search_card", default =False,
                 help = ( "Search icafe + card."   ) )
...
group.add_option( "-c", "--changelist", action = "store", dest = "change_list",
                 metavar = "CHANGE_LIST", default = None,
                 help = "Specify the change list to review." )
...
group.add_option( "--env_encoding", action = "store",
                 dest = "env_encoding", default = 'utf-8',
                 help = ( "Specify the console encoding."   ) )
...
class VersionControlSystem( object ):
    ...
    def UploadBaseFiles( self, issue, rpc_server, patch_list, patchset, options,
        ...
        def UploadFile( filename, file_id, content, is_binary, status, is_base,workcopy_content=None):
            ...
            if not file_too_large and workcopy_content is not None:
        ...
        for filename in patches.keys():
            ...
            is_new_version = (file_id_str.find( "new" ) != -1)
            if file_id_str.find( "nobase" ) != -1 or is_new_version:
                base_content = None
                file_id_str = file_id_str[file_id_str.rfind( "_" ) + 1:]
            file_id = int( file_id_str )
            if base_content is None:
                if is_new_version:
                    UploadFile( filename, file_id, "", is_binary, status, False,new_content or "" )
            else:
                UploadFile( filename, file_id, base_content, is_binary, status, True,new_content or "")
            time.sleep(0.01)
...
class SubversionVCS( VersionControlSystem ):
    ...
    def GuessVersion(self):
        ...
        if info.splitlines(True)[0].find('1.7')!=-1 or info.splitlines(True)[0].find('1.8')!=-1:
    ...
    def GenerateDiff( self, args ):
        ...
        if url_revision:
            ...
            if self.options.change_list:
                cmd += ["--changelist",self.options.change_list]
            cmd.extend( args )
        data = RunShell( cmd )
        count = 0
    ...
    def GetBaseFile( self, filename ):
        ...
        if status[0] == "A" :
            ...
            if self.rev_end and not self.tagdiff:
                url = "%s/%s@%s" % ( self.svn_base, filename,self.rev_end)
                new_content = RunShell( ["svn", "cat", url,"-r%s"%self.rev_end],
                                                                universal_newlines = True, 
                                                                silent_ok = True )
            elif self.tagdiff:
                rev_end = self.rev_end.split( '@' )
                ...
                if len( rev_end ) == 2:
                    url = "%s/%s@%s" % ( rev_end[0], filename,rev_end[1])
                    command.append(url)
                    command.append("-r%s"%rev_end[1])
                else:
                    url = "%s/%s" % ( self.rev_end, filename )
                    command.append(url)
                new_content = RunShell(command,
                                                                universal_newlines = True,
                                                                silent_ok = True )
            else:
            ...
            if get_base:
                if is_binary:
                    ...
                    universal_newlines = True
                if self.rev_end and not self.tagdiff and status[0] != "D":
                    url = "%s/%s@%s" % ( self.svn_base, filename,self.rev_end)
                    new_content = RunShell( ["svn", "cat", url,"-r%s"% self.rev_end],
                                                                  universal_newlines = universal_newlines,
                                                                  silent_ok = True )
                elif self.rev_end and self.tagdiff and status[0] != "D":
                    rev_end = self.rev_end.split( "@" )
                    command = []
                    command.append("svn")
                    command.append("cat")
                ...
                if self.rev_start and not self.tagdiff:
                    ...
                    url = "%s/%s@%s" % ( self.svn_base, filename,self.rev_start)
                    base_content = RunShell( ["svn", "cat", url,"-r%s"%self.rev_start],
                                            universal_newlines = universal_newlines,
                                            silent_ok = True )
                    ...
                    if len( rev_start ) == 2:
                        ...
                        command.append(url)
                        command.append("-r%s"%rev_start[1])
                    else:
                        url = "%s/%s" % ( rev_start[0], filename )
                        command.append(url)
                    base_content = RunShell( command,
                                                                   universal_newlines = universal_newlines,
                                                                    silent_ok = True )
                    base_content = self.DecodingAndEncodingByFile( base_content, filename )
                else:
...
def _close_issue( options ):
    rpc_server = GetRpcServer( server = options.server,
                               email = options.email,
...
def check_out( httpserver , options ):
    response_body = httpserver.Send( "/getversionid/%s/" %options.check_out )
    if response_body:
        files = response_body.splitlines()
        for file in files:
            version_key,filename = file.split(' ')
            content,status = HttpService().get("http://" + version_key)
            if not status:
                print "file %s is damaged."%filename
                continue
            if filename.rfind('/') != -1:
            ...
            f.write(content)
            f.close()
            print "file %s is download."%filename
    print "check out down"
...
def RealMain( argv, data = None ):
  ...
  """
    ...
    cardquery_service = CardQueryService(rpc_server)
    form_fields = []
    ...
    if not isNewVer:
        ...
        print "You can get the newest upload.py from here:" + DEFAULT_REVIEW_SERVER + "/dynamic/fc/upload.py"
        prompt = "Are you sure to continue?(y/N) "
    ...
    if options.revision_review:
        if options.revision:
            ErrorExit( "--full_review can't used with --rev parameter." )
    ...
    if not isinstance( vcs, SubversionVCS ):
        if options.revision_review or options.listandpatch:
            ErrorExit( "params '-l,--full_review' is only for svn!" )
    ...
    if base:
        ...
        if not options.issue and api_service.need_add_card(base):
            ...
            space = find_space(base)
            if space:
                cards = cardquery_service.lastest_created_card(space)
                print 'icafe card created by you latest'
                sequence = 'sequence'
                #win
                if not use_shell:
                    sequence = RichTextPrint.render_green_text('sequence')
                #if we want to display correctly in ssh terminal,we
                #export PYTHONIOENCODING=utf-8 or PYTHONIOENCODING=gbk
                print '%s  %s  %s  %s'%(sequence,'status','createdate','subject')
                try:
                    if cards:
                        for card in  cards:
                            card_sequence = '%s-%d'%(space,card['sequence'])
                            #win
                            if not use_shell:
                                card_sequence  = RichTextPrint.render_green_text(card_sequence)
                            print "%s  %s  %s  %s"%(card_sequence,card['status'],card['createdTime'],card['title'])
                except Exception,ex:
                    logger_service.error(str(ex))
                    print "[ERROR]upload.py can not display the latest icafe cards,please set the console encoding to utf-8 or gbk"
            if  not options.card:
                q_highlight = "-q"
                #win
                if not use_shell:
                    q_highlight  = RichTextPrint.render_green_text(q_highlight)
                check_result = "[WARN] You need bind the story,please input it,input %s to search card\nSTORY:"%q_highlight
                answer = raw_input( check_result ).strip()
                while not answer:
                    check_result = "[WARN] the input is null ,please input it,input %s to search card\nSTORY:"%q_highlight
                    answer = raw_input( check_result ).strip()
                if answer=='-q':
                    options.search_card = True
                else:
                    options.card = answer
    
    if options.search_card:
        continue_search = True
        while continue_search:
            search_highlight = "SEARCH"
            #win
            if not use_shell:
                search_highlight = RichTextPrint.render_red_text(search_highlight)
            subject = raw_input( "[INFO]please input subject to search the card\n%s:"%search_highlight ).strip()
            try:
                subject = subject.decode(options.env_encoding).encode('utf-8')
            except:
                if options.env_encoding=="utf-8":
                    subject = subject.decode('gbk','ignore').encode('utf-8','ignore')
                    options.env_encoding = 'gbk'
            cards = cardquery_service.search_card(subject)
            print 'search result'
            sequence = 'sequence'
            #win
            if not use_shell:
                sequence = RichTextPrint.render_green_text('sequence')
            print '%s  %s  %s'%(sequence,'createdate','subject')
            try:
                if cards:
                    for card in  cards:
                        card_sequence = '%s-%d'%(card['space'],card['sequence'])
                        #win
                        if not use_shell:
                            card_sequence  = RichTextPrint.render_green_text(card_sequence)
                        print "%s  %s  %s"%(card_sequence,card['createTime'],card['title'])
                else:
                    print "no result"
            except Exception,ex:
                logger_service.error(str(ex))
                print "[ERROR]upload.py can not display search results,please set the console encoding to utf-8 or gbk"
            q_highlight = "-q"
            #win
            if not use_shell:
                q_highlight  = RichTextPrint.render_green_text(q_highlight)                
            answer = raw_input( "choose a card sequeue,input %s continue to search\nSTORY:"%q_highlight).strip()
            if answer!='-q':
                options.card = answer
    ...
    try:
        message = message.decode(options.env_encoding ).encode( 'utf8')
    except Exception:
        if options.env_encoding=="utf-8":
            message = message.decode('gbk','ignore').encode('utf-8','ignore')
            options.env_encoding = 'gbk'
        pass
    if options.revision_review:
    ...
    if description:
        try:
            description = description.decode(options.env_encoding ).encode( 'utf8' )
        except Exception:
            if options.env_encoding=="utf-8":
                description = description.decode('gbk','ignore').encode('utf-8','ignore')
                options.env_encoding = 'gbk'
            pass
    ...
    for file, info in files.iteritems():
        ...
        if not info[1] is None:
            checksum = md5( info[1] ).hexdigest()
        else:
            checksum = md5( "" ).hexdigest()
        if version_hashes:
            version_hashes += "|"
        version_hashes += checksum + ":" + info[3] + ':' + file
    form_fields.append( ( "base_hashes", base_hashes ) )
    form_fields.append( ( "version_hashes", version_hashes ) )
    if options.public:
        if options.issue:
  ...
  not response_body.startswith( "Issue updated." ):
    ...
    if need_check_syle_result['result']==1:
...
class OpenSourceHandler(object):
    ...
    def check_has_modify(self):
        ...
        if not info or not info.strip():
            return False
        return True
class CardQueryService(object):
    def __init__(self,rpc_server):
        # the logined server
        self.rpc_server = rpc_server
    def lastest_created_card(self,space):
        response_body = self.rpc_server.Send( "/card/space/latest?space=%s"%space)
        result = JsonReader().read(response_body)
        if result['status'] == 0:
            print '[ERROR]%s'%result['msg']
            return None
        return result['cards']
    def search_card(self,subject):
        quoted_subject = urllib.quote(subject)
        response_body = self.rpc_server.Send( "/card/query?subject=%s"%quoted_subject)
        result = JsonReader().read(response_body)
        if result['status'] == 0:
            print '[ERROR]%s'%result['msg']
            return None
